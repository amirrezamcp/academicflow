...
class UpdateMasterRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if (Schema::hasColumn('masters', 'email')) {
            $rules['email'] = [
                ...
                'email',
                'max:100',
                Rule::unique('masters')->ignore($masterId),
            ];
        }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // اعتبارسنجی سفارشی برای تخصص‌ها
            if ($this->specialties && strlen($this->specialties) > 500) {
                $validator->errors()->add('specialties', 'تعداد کاراکترهای تخصص‌ها بیش از حد مجاز است.');
            }
            ...
            if ($this->phone) {
                ...
                if (strlen($phone) < 10 || strlen($phone) > 11) {
                    $validator->errors()->add('phone', 'شماره تلفن باید ۱۰ یا ۱۱ رقم باشد.');
                }
            }
        });
    }
